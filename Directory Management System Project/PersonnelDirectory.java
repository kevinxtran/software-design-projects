...
public class PersonnelDirectory
{
  ...
  private static void displayMenu()
  {
	...
	System.out.println("\n\t Select one of the options above (1, 2, 3, 4, or 5)");
  }
  ...
  private static void addPersonnel(Scanner scan, Personnel per, totalObjects total) {
	...
	double salary;
	// Phase 2 Part 1 & 3
	// Adjusting addPersonnel method to add execs, security, and volunteers
	...
	// Who are you?
	System.out.println("What type of employee are you? (Executive, Volunteer, or Security)");
	String type = scan.nextLine().toLowerCase();

	// Use PersonnelFactory now 
	...
	*/
	...
	// Employee inherits from the person class, we will use personnel to save time as it manages the people
	Person newPersonnel = factory.createPersonnel(type, lastN, firstN, middleN, empID, salary);

	if (newPersonnel != null){
		per.addPersonnel(newPersonnel);
		total.objectAdded();
		System.out.println(type+" was successfully added!");
	}else{
		System.out.println("Invalid response, failed to add personnel. Please try again.");
	}
